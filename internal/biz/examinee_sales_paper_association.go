...
func (uc *ExamineeSalesPaperAssociationUseCase) ExamQuestion(ctx context.Context, req *v1.ExamQuestionRequest) (resp *v1.ExamQuestionResponse, err error) {
	...
	salesPaper, err := uc.salesPaperCase.GetSalesPaperDetail(ctx, associationId)
	if err != nil {
		l.Errorf("ExamQuestion.salesPaperCase.GetSalesPaperDetail Failed, req:%v, err:%v", req, err.Error())
		err = innErr.ErrInternalServer
		return
	}
	if salesPaper == nil {
	...
	resp, err = uc.questionUseCase.ExamQuestion(ctx, req, salesPaper.ID)
	if err != nil {
		l.Errorf("ExamQuestion.questionUseCase.ExamQuestion Failed, req:%v, salesPaperId:%v, err:%v", req, salesPaper.ID, err.Error())
		err = innErr.ErrInternalServer
		return
	}
	return
